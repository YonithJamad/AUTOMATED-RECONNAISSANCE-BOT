...
def get_cves(host_data):
    ...
    results = []
    # Use ThreadPoolExecutor to fetch CVE details in parallel
    with ThreadPoolExecutor(max_workers=10) as executor:
        future_to_cve = {executor.submit(get_single_cve_detail, cve_id, nmap_score): cve_id for cve_id, nmap_score in unique_vulnerabilities}
    ...
    try:
        ...
        if host not in nm.all_hosts():
            if len(nm.all_hosts()) > 0:
                ...
                return {"error": "Host not found or down during initial sweep"}

        # Extract only the OPEN ports from the discovery phase
        open_ports = []
        host_data = nm[host]
        for proto in host_data.all_protocols():
        ...
        if not open_ports:
            # If absolutely no open ports are found, we can return the basic info immediately
            return {
                "network_scan": {
